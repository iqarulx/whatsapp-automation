...
// On hosts where the system is missing shared libraries Chrome needs (common
// on shared hosting with no root access), set USE_BUNDLED_CHROMIUM=1 to use
// @sparticuz/chromium's self-contained build instead of Puppeteer's default
// Chrome. Left unset, this keeps using Puppeteer's normal Chrome (what local
// dev on Windows/macOS/a full Linux box already has working).
async function buildPuppeteerOptions() {
  ...
  if (process.env.USE_BUNDLED_CHROMIUM === "1") {
    ...
    // @sparticuz/chromium always extracts its binary into os.tmpdir(),
    // which defaults to the real /tmp. Some shared hosts mount /tmp
    // noexec, so nothing extracted there can actually run. os.tmpdir()
    // respects TMPDIR, so point it at a writable+executable folder inside
    // the project instead.
    const chromiumTmpDir = process.env.CHROMIUM_TMPDIR || path.join(__dirname, "..", ".cache", "chromium-tmp");
...
function initClient(userId) {
  ...
  (async () => {
    ...
    console.error(`[whatsapp:${userId}] client setup failed:`, err);
    setState(userId, { status: "auth_failure", error: err.message });
    const s = sessions.get(userId);
    if (s) s.client = null;
  });
